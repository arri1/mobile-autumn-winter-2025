...
export default function CreatePostScreen() {
  const { createPost, updatePost, isLoading, currentPost, clearCurrentPost } = usePostsStore();
  ...
  // Очистка формы при монтировании, если это создание нового поста
  useEffect(() => {
    // Если это экран создания нового поста, очищаем все
    if (activeScreen === 'createPost') {
      resetForm();
      clearCurrentPost();
    }
    
    // Если это экран редактирования и есть currentPost, заполняем форму
    if (currentPost && activeScreen === 'editPost') {
      setIsEditMode(true);
